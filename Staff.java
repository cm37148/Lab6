...
public static void main(String[] args) {
       ...
       int count = staff.writeToTextFile("EmployeeToStringsData.txt");
       System.out.println("The number of employees' information saved was "+count);
	}
	...
	public double calcPayForAllEmployees() {
	  ...
	  public int readFromTextFile(String filenameAndPath) {
	        ...
	        try {//start of outer try block
	            //open the text file
	            FileReader fr = new FileReader(filenameAndPath); //Creates a new FileReader, given the name of the file to read from.
	            BufferedReader br = new BufferedReader(fr);//Reads text from a character-input stream, buffering characters so as to provide for the efficient reading
	            ...
	            while(aLineFromTheTextFile!=null) {//loop until readLine() returns null
	                	 aLineFromTheTextFile = br.readLine();
	            try {//start of our inner try block
	                    String [] dataForASingleEmployee = aLineFromTheTextFile.split(",");//one line is one employee's info. Fields are separated by # symbol
	                    ...
	                    double hoursWorked = Double.parseDouble(dataForASingleEmployee[3]); // fourth field is hours worked for the period
	                    //TempWorker only needs these 4 fields so if we have only 4 fields then this is a TempWorker
	                    ...
	                    //StudentWorker needs the 4 previous fields plus 6 others, so if we have 10 fields then this is a StudentWorker
	                    ...
	                    else {//we did not have exactly 4 or 10 fields to consider the line useful for creating a StudentWorker or TempWorker
	                        System.out.println("Did not have exactly 4 or 10 fields to consider the line useful for creating an Employee"+(++countOfBadLines));
	                    }//end else
